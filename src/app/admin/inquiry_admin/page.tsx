'use client';
import { AdminInquirySort } from './components/AdminInquirySort';
...
const adminInquiryList = [
	{
		...
		inquirytime: '2024/11/21(月)/10:25:30',
		Status: '未対応',
	},
	{
		id: 2,
		...
		inquirytime: '2024/11/22(月)/10:25:30',
		Status: '対応済',
	},
	{
		id: 3,
		...
		inquirytime: '2024/11/23(月)/10:25:30',
		Status: '未対応',
	},
	{
		id: 4,
		...
		inquirytime: '2024/11/24(月)/10:25:30',
		Status: '対応済',
	},
	{
		id: 5,
...
function Page() {
	const title = '問い合わせ管理';
	const [data, setData] = useState(adminInquiryList); // データの状態管理
	const [sortOrder, setSortOrder] = useState<'asc' | 'desc'>('asc'); // ソート順の状態

	const handleDateSort = () => {
		const newSortOrder = sortOrder === 'asc' ? 'desc' : 'asc'; // 昇順と降順を切り替え
		const sortedData = sortByDate(data, 'inquirytime', newSortOrder); // ソート処理
		setData(sortedData);
		setSortOrder(newSortOrder); // ソート順を更新
	};
	...
	return (
		<Box w={'1200px'} m={'auto'}>
			<Box padding={'20px'}>
				...
				<AdminInquirySort onDateSort={handleDateSort} />{' '}
				{/* 日付ソートの関数を渡す */}
				<AdminInquiryAccordion data={data} /> {/* ソート済みデータを渡す */}
			</Box>
		</Box>
	);
}
